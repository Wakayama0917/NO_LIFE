...
// スタティック　メンバー
//-----------------------------------------------------------------------------
const char* Application::WINDOW_TITLE = "NO LIFE";
...
unsigned long Application :: MainLoop()
{
	...
	// ゲームの初期処理
	Game_Manager::GetInstance()->GameInit();
	//game_manager.GameInit();

	// タイマ解像度をミリ秒に
	::timeBeginPeriod(1);

	while ((window->ExecMessage()) && (Game_Manager::GetInstance()->GameFlg)) {
		...
		Game_Manager::GetInstance()->GameAllUpdate(delta_time);
		/*Game_Manager::GetInstance()->GameInput(delta_time);
		Game_Manager::GetInstance()->GameUpdate(delta_time);
		Game_Manager::GetInstance()->GameRender();*/
		//game_manager.GameInput(delta_time);		// ｹﾞｰﾑ入力	
		//game_manager.GameUpdate(delta_time);		// ｹﾞｰﾑ更新
		//game_manager.GameRender(delta_time);		// ｹﾞｰﾑ描画
...
//			printf("sleep:%f \n", tt);
	...
	// ゲームの終了処理
	Game_Manager::GetInstance()->GameDispose();
	//game_manager.GameDispose();
