...
// 法線ベクトルを計算
// ノーマライズ
void Sphere::Normalize(XMFLOAT3 _vector, XMFLOAT3& _Normal) {
	DX11Vec3Normalize(_Normal,_Normal);
}
...
void Sphere::Draw(ID3D11DeviceContext* _device) {
	...
	unsigned  offset = 0;
	_device->IASetVertexBuffers(0, 1, m_pVertexBuffer.GetAddressOf(), &stride, &offset);

	_device->IASetIndexBuffer(m_pIndexBuffer.Get(), DXGI_FORMAT_R32_UINT, 0);			// インデックスバッファをセット
	_device->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);		// トポロジーをセット（旧プリミティブタイプ）
	_device->IASetInputLayout(m_pVertexLayout.Get());					// 頂点レイアウトセット

	_device->VSSetShader(m_pVertexShader.Get(), nullptr, 0);			// 頂点シェーダーをセット
	_device->PSSetShader(m_pPixelShader.Get(), nullptr, 0);			// ピクセルシェーダーをセット
...
bool Sphere::Init(float _r,				// 半径
	int _division_horizontal,			// 水平方向の分割数
	int _division_vertical,				// 垂直方向の分割数
	XMFLOAT4 _color,
	ID3D11Device* _device) {

	bool sts;
		// 分割数を保存
	m_divX = _division_horizontal;
	m_divY = _division_vertical;
	...
	sts = CreateVertexShader(_device,
