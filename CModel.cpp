...
void CModel::Draw(XMFLOAT4X4& mtxworld) {
	...
	// 頂点フォーマットをセット
	//m_pVertexLayout = ModelMgr::GetInstance().GetModelPtr("assets/fbx/Dodai/Dodai.fbx")->m_pVertexLayout;
	devcontext->IASetInputLayout(m_pVertexLayout);
	// 頂点シェーダーをセット
	//m_pVertexShader = ModelMgr::GetInstance().GetModelPtr("assets/fbx/Dodai/Dodai.fbx")->m_pVertexShader;
	devcontext->VSSetShader(m_pVertexShader, nullptr, 0);
	// ピクセルシェーダーをセット
	//m_pPixelShader = ModelMgr::GetInstance().GetModelPtr("assets/fbx/Dodai/Dodai.fbx")->m_pPixelShader;
