...
public:
	...
	bool Init(const std::vector<MyVertex>& _v) {
		m_vsize = _v.size();
		...
		// 頂点バッファ生成
		sts = CreateVertexBufferWrite(GetDX11Device(), sizeof(MyVertex), _v.size(), (void*)_v.data(), &m_vbuffer);
		if (!sts) {
	...
	void SetVertex(std::vector<MyVertex>& _v) {

		D3D11_MAPPED_SUBRESOURCE pData;

		HRESULT hr = GetDX11DeviceContext()->Map(m_vbuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &pData);
		if (SUCCEEDED(hr)) {
			memcpy_s(pData.pData, pData.RowPitch, (void*)(_v.data()), sizeof(MyVertex) * _v.size());
